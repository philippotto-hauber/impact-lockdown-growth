...
oecd_sect_raw %>% select(codes, G.I = `G-I`, R.U = `R-U`) -> oecd_sect
```
...
df %>% select(-c(dev_gdp)) %>% 
       ...
       ggplot(aes(x = value, y = gdpH1))+
         geom_point()+
         geom_text(data = filter(df_R2, var %in% c("Lockdown-Stringenz", "Offenheit", "Tourismus (WTTC)", "Tourismus (Weltbank)", "Covid-19-Infektionen", "Covid-19-Todesfälle")),
         ...
         labs(y = "BIP, Prozent", x = "",
              ...
              subtitle = "Zusammenhang mit Covid-19, Grad der Offenheit, Lockdown-Stringenz und Bedeutung des Tourismus ",
...
df %>% filter(var %in% c("retail_rec", "transit", "workplace"),
  ...
  geom_point(size = 3)+
  geom_text(data = filter(df_R2, var %in% c("retail and recreation", "transit stations", "workplace")), 
            aes(x = x, y = y, label = R2), size = 4)+
...
## OECD data on economic structure vs. growth in 2020H1


```{r}
df %>% filter(var %in% c("G.I", "R.U"),
              country != "China") %>%
        mutate(status = ifelse(country %in% list_EME, "Schwellenländer", "fortgeschrittene Volkswirtschaften"),
               var = fct_recode(var, "trade, transport, accommodation" = "G.I", 
                                     "arts, entertainment, recreation" = "R.U"
                                )
               ) -> df_plot
  ...
  geom_smooth(method = "lm", se = FALSE, color = "black", size = 0.5)+
  geom_text(data = filter(df_R2, var %in% c("trade, transport, accommodation", "arts, entertainment, recreation")),
  ...
  labs(y = "BIP, Prozent", x = "",
      title = "Covid-19-Pandemie: wirtschaftliche Entwicklung in 2020H1",
      subtitle = "Zusammenhang mit Wirtschaftsstruktur ",
      caption = paste0("Daten für ", length(unique(df_plot$country)) - 1 , " Länder, BIP: Veränderung zwischen 2020Q2 und 2019Q4, Anteil an der Bruttowertschöpfung.Quelle: OECD National Accounts."))+
        theme_minimal()+
        theme(legend.title = element_blank(), legend.position = "top", legend.text=element_text(size=9))
```
```{r}
ggsave(
  filename = "Abb_BIPH1_Bereiche.pdf",
  plot = last_plot(),
  device = "pdf",
  path = NULL,
  scale = 1,
