...
```{r}
oecd_sect_raw <- read.xlsx(file = "IWH1.xlsx", sheetName = "OECD Data")

oecd_sect_raw %>% select(codes, G.I, R.U) -> oecd_sect
```
...
df %>% filter(var %in% c("G.I", "R.U"),
  ...
  geom_smooth(method = "lm", se = FALSE, color = "black", size = 0.5)+
  stat_regline_equation(formula = y ~ x, aes(label =  paste(..eq.label.., ..rr.label.., sep = "~~~~")),
                        label.x.npc = "left",
                        label.y.npc = "top",)+
  geom_point(aes(color = status), size = 3)+
  ylim(c(-25, 0))+
